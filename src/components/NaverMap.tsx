...
function buildMarkerContent(shop: Restaurant, lang: Lang): string {
  const T = t[lang];
  ...
  const priceText =
    shop.minPrice != null
      ? `${shop.minPrice.toLocaleString()}${T.priceUnit}`
      : "";
...
const NaverMap: React.FC<NaverMapProps> = ({
  ...
  useEffect(() => {
    ...
    restaurants.forEach((shop) => {
      ...
      if (!marker) return;
      marker.setIcon({
        content: buildMarkerContent(shop, lang),
