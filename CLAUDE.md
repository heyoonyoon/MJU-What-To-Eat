...
| Keyword | Target Sub-Agent | Instructions |
| :--- | :--- | :--- |
| **"설계"**, **"Design"**, **"Model"** | `designer.md` | Proactively read `CLAUDE_TASK.md` and generate `DESIGN_MODEL.md`. |
| **Requirement** | `planner.md` | **FIRST CALL.** Analyze and generate `CLAUDE_TASK.md`. |
| **"분석"**, **"Audit"** | `auditor.md` | (Future) Logic and architecture deep-dive. |

### 🛠️ Execution Protocol
1. **Identify**: Check the user's command.
2. **Prioritize Planner**: Always load `planner.md` first for any request that changes the codebase.
3. **Execute**: Perform the task following the sub-agent's protocol and project constraints.

---
*Environment Note: Local Node/icu4c mismatch prevents CLI runtime execution. Use browser for verification.*
